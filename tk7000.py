...
def load7000():
    ...
    try:
        ...
        for record in results:
            allwords.append(record[0].encode('utf8'))
        db.commit()
        # 關閉連線
        db.close()
...
class SampleApp(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.frames = {}
        for F in (StartPage, RegistPage, Menu,gamepage,letterpage,person,person_letter,person_word,topten,topten_letter,topten_word,wordpage):
...
class gamepage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        button = tk.Button(self, text="單字",command=self.goword, font=BTN_FONT,background="lightblue",foreground="maroon")
        #button.grid(row=2,columnspan=4)
        button.place(x=400, y=350, anchor=CENTER)
