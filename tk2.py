...
def load7000():
    ...
    try:
        ...
        results = cursor.fetchall()
        for record in results:
            words_7000.append(record[0].encode('utf8'))
        db.commit()
        # 關閉連線
        db.close()
...
class SampleApp(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.frames = {}
        for F in (StartPage, RegistPage, Menu,gamepage,person,person_letter,person_word,topten,topten_letter,topten_word,wordspage,wordpage):
...
class wordspage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        #label.grid(row=1,columnspan=4)
        label.place(x=400, y=200, anchor=CENTER)
        button = tk.Button(self, text="7000單",command= lambda:self.goword(0),font=BTN_FONT,background="lightblue",foreground="maroon")
        #button.grid(row=2,columnspan=4)
        button.place(x=400, y=275, anchor=CENTER)
        button = tk.Button(self, text="自訂",command= lambda:self.goword(1),font=BTN_FONT,background="lightblue",foreground="maroon")
        #button.grid(row=2,columnspan=4)
        button.place(x=400, y=350, anchor=CENTER)
...
class wordpage(tk.Frame):
    ...
    def run(self):
        ...
        while True and heart>0:
            global positions
            input_text= self.font.render(word_input, True, lightblue)
